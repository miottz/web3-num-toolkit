...
export const _convertScientificToNumberString = (num: number): string => {
  if (!String(num).includes('e')) return String(num)
  return num.toFixed(20).replace(/\.?0+$/, '')
}

export const _findFirstNonZeroIndex = (str: string): number => {
 ...
 * @returns 
 */
export function formatPriceDecimals(number: number | string): string {
  const num = Number(number || 0)
  if (num < 1) {
    const decimalPart =
      _convertScientificToNumberString(num).split('.')[1] ??
      _convertScientificToNumberString(num).split('.')[0]
    const nonZeroIndex: number = _findFirstNonZeroIndex(decimalPart)
    if (nonZeroIndex === -1) {
      return '-'
    } else {
      let count = 1
 ...
 * @return {*}
 */
export function formatVolumeDecimals(value: number | string) {
 ...
 * @return {*}
 */
export function formatVolumeDecimalsZh(value: number | string) {
 ...
 * @return {*}
 */
export function formatPercentageDecimals(value: number | string): string {
 ...
 * @return {*}
 */
export function thousandSeparatorNum(value: number | string) {
 ...
 * @return {*}
 */
export function formatVolumeInt(value: number | string) {
  const num = Number(value) || 0
  // 数据小于 1,000,000
  if (num < 1000000) {
 ...
 * @return {*}
 */
export function formatVolumeIntZh(value: number | string) {
  const num = Number(value) || 0
 ...
 * @return {*}
 */
export function formatMinimumDecimals(value: number | string, foldingBoundary: number = 4): string {
  if (!Number(value)) {
    return '0'
  }
  ...
  if (strValue.includes('e')) {
    ...
    decimalPart =
      _convertScientificToNumberString(Number(strValue)).split('.')[1] ??
      _convertScientificToNumberString(Number(strValue)).split('.')[0]
  }
  const zeroCount = decimalPart.match(/^0+/)?.[0].length || 0
 ...
 */
export function fixedToInt(value: string | number, precision: number = 18): string {
 ...
 */
export function intoFixed(value: string | number, precision: number = 18): string {
  if (value === null || value === undefined) {
    throw new Error('Invalid number')
  }
 ...
 * @return {*}
 */
export function formatHash(hash: string, startLen?: number, endLen?: number | null) {
  if (!hash) {
    return hash
  }
